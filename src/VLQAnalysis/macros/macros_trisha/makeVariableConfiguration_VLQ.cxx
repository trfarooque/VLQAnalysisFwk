...
int main(int argc, char** argv){
  ...
  while ((optind < argc) && (argv[optind][0]=='-')) {
    ...
    else if  (_switch == "--opt_splitemu")  { ss >> opt_splitemu; } //_el and _mu channels in 1L if opt_splitemu==TRUE
  ...
  std::cout<<"opt_splitemu      =  " << opt_splitemu     << std::endl;
  ...
  if(opt_reg == "0L_presel"){ m_prefix = {"c0lep6jin2bin","c0lep6jin2binHighMtbmin","c0lep6jin2binLowMtbmin",
					  "c0lep6jin2bex","c0lep6jin2bexHighMtbmin","c0lep6jin2bexLowMtbmin",
					  "c0lep6jin1bex","c0lep6jin0bex",
					  "c0lep6jex2bin","c0lep6jex2binHighMtbmin","c0lep6jex2binLowMtbmin",
					  "c0lep6jex2bex","c0lep6jex2bexHighMtbmin","c0lep6jex2bexLowMtbmin",
					  "c0lep6jex1bex","c0lep6jex0bex" }; }
  else if(opt_reg == "1L_presel"){ m_prefix = {"c1lep5jin2bin","c1lep5jin2bex","c1lep5jin1bex","c1lep5jex2bin","c1lep5jex2bex","c1lep5jin1bex"}; }
  ...
  std::cout<<" Before splitemu number of regions = "<<m_prefix.size()<<std::endl;
  if(opt_splitemu=="TRUE"){
    std::vector<std::string> temp_prefix = m_prefix;
    for(std::string prefix : m_prefix){
      temp_prefix.push_back(prefix);
      if(prefix.find("c1lep")!=std::string::npos){ 
	temp_prefix.push_back(prefix+"_el"); 
	temp_prefix.push_back(prefix+"_mu"); 
      }
    }
    m_prefix.clear();
    m_prefix = temp_prefix;
  }

  std::cout<< " After splitemu number of regions = "<<m_prefix.size()<<std::endl;

  //, "mbb_mindR",, "el_n", "mu_n", "lep_n",, "RCTTMass_jets_n","dPhi_lepmet"
  ...
  for(unsigned int r = 0; r < m_prefix.size(); r++){
    ...
    var_title = "#splitline{#splitline{#scale[1.2]{#bf{#it{ATLAS}} Internal}}{#sqrt{s}=13 TeV, 36.5 fb^{-1}}}{" + reg_label + "}"; 
    /*
