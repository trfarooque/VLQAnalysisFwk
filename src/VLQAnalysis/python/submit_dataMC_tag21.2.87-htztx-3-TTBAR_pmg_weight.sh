...
do
	...
	do
		...
		python Submit_VLQAnalysis_new.py dryrun=false producetarball=true campaign=${campaign} --sleep=2 \
		    ...
		    --computeWeightSys=true --DOEXPSYS=false --DOTHEORYSYS=true \
		    --ISAFII=false \
		    --MAXMETCUTTWOLEP=100. \
		    --runSignals=false --runOtherBkgd=false --runTtbar=true --runQCD=false --runData=false \
		    --runTopEW=false --runDibosons=false --runSingletop=false --runZjets=false --runWjets=false \
