...
bool VLQ_Analysis_Data2015::SumAnalysisRegions(const bool newFile){
  ...
  */
  ...
  if(is_singletop){
    ...
    DRDSPropagator->Init(m_selector -> GetSelectionTree(), std::getenv("BUILDDIR")+std::string("/x86_64-centos7-gcc8-opt/data/VLQAnalysis/single_top_syst.root"));
    //DRDSPropagator->Init(m_selector -> GetSelectionTree(), std::getenv("ROOTCOREBIN")+std::string("/data/VLQAnalysis/single_top_syst.root"));
